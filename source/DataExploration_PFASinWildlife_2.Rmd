...
#Set directory
```{r pressure, echo=FALSE}
setwd("Z:/SCI/BEES/MMR/Students_Postdocs/Cat/Ideel-Lab_2/PFAS_docus/SystMap_PFASconcWildlife/Data_Analysis")
```
...
pfas_species_heat_4 <- pfas_species_heat_3 %>% ggplot(aes(reorder(Scientific_name, -n), reorder(PFAS_name, -n),  fill = n)) + 
  geom_tile() + scale_fill_gradient(low = "purple", high = "pink", na.value = NA) + theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(y="PFAS Names", x = "Scientific species name", fill = 'Count')
pfas_species_heat_4
...
sankeyNetwork_plot <- sankeyNetwork(Links = sankey_author, Nodes = nodes,
                   Source = "IDsource", Target = "IDtarget", Value = "n", NodeID = "name", 
                   sinksRight=FALSE)
...
#Make the Network
sankeyNetwork_plot_2 <- sankeyNetwork(Links = sankey_author, Nodes = nodes,
                   Source = "IDtarget", Target = "IDsource", Value = "n", NodeID = "name", 
                   sinksRight=FALSE)
                   
sankeyNetwork_plot_2

#Save figure

# save it as an html
saveNetwork(sankeyNetwork_plot_2, "sankeyNetwork_plot_2.html")

# you convert it as png
webshot("sankeyNetwork_plot_2.html","sankeyNetwork_plot_2.png")
