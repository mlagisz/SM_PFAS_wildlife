---
title: "DataExplorations_PFASWildlife"
...
#Set directory
```{r pressure, echo=FALSE}
setwd("Z:/Ideel-Lab_2/PFAS_projects/SystMap_PFASconcWildlife/Data_Analysis")
...
Species_distinct_2 <- Species_distinct %>% distinct(Study_ID, Lay_name,  .keep_all = TRUE)
dim(Species_distinct_2)
#2355    3

Species_distinct_2 %>% distinct(Scientific_name)
Species_distinct_2 %>% distinct(Lay_name)

table_species <- Species_distinct_2 %>%
...
#ggsave("Publication_year_bar.jpg", plot = Publication_year_bar, device = 'jpg', width = 168, height = 100, units = "mm",
...
PFAS_Compounds_long_short <- 
  ggplot(data = PFAS_Compounds_21_short,
         aes(x=reorder(PFAS_name, -n),
             y=n,
             fill = Chain_length)) + 
  geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1)) + labs(y="Count", x = "PFAS compounds", fill = 'Carbon chain length') + theme_minimal() + coord_flip()+
  theme(legend.position = "top")

PFAS_Compounds_long_short

#ggsave("PFAS_Compounds_long_short.jpg", plot = PFAS_Compounds_long_short, device = 'jpg', width = 168, height = 100, units = "mm",
#    dpi = 300, limitsize = T)
...
species_bar <- Species_distinct_10 %>% 
  group_by(Scientific_name) %>%
  summarise(n = n()) %>%
...
Species_traits_10 <- Species_traits[Species_traits$Scientific_name %in% names(which(table(Species_traits$Scientific_name) > 10)), ]
dim(Species_traits_10)
#506   13

Higher_taxon <- Species_traits_10[,c(2,4)]

species_bar <- Species_traits_10 %>% 
  group_by(Scientific_name) %>%
  summarise(n = n()) 
...
species_bar_4 <- species_bar_3 %>% ggplot(aes(x=reorder(Scientific_name, -n), y=n, fill = Species_higher_taxon)) + theme_minimal() + geom_bar(stat="identity", colour="gray", group = 'Species_higher_taxon') +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(y="Count", x = "Scientific species name", fill = "Class")  +
  coord_flip() + 
  theme_minimal() +
  theme(legend.position = "top")+
...
#ggsave("species_bar_4.jpg", plot = species_bar_4, device = 'jpg', width = 168, height = 100, units = "mm",
      dpi = 300, limitsize = TRUE)


#Bar plot IUCN status
IUCN_bar <- Species_traits %>% 
  group_by(redlistCategory) %>%
  summarise(n = n()) %>%
...
#ggsave("pfas_habitat_heat.jpg", plot = pfas_habitat_heat, device = 'jpg', width = 168, height = 100, units = "mm", dpi = 300, limitsize = TRUE)
```
...
#ggsave("pfas_biogeographic_heat.jpg", plot = pfas_biogeographic_heat, device = 'jpg', width = 168, height = 100, units = "mm",
#       dpi = 300, limitsize = TRUE)
```
...
pfas_species_heat_4 <- pfas_species_heat_3 %>% ggplot(aes(reorder(Scientific_name, -n), reorder(PFAS_name, -n),  fill = n)) + 
  geom_tile() + scale_fill_gradient(low = "white", high = "blue", na.value = NA) + theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), axis.text.y = element_text(size = 7)) + labs(y="PFAS compounds", x = "Scientific species name", fill = 'Count')
pfas_species_heat_4
...
#ggsave("pub_year_geograph_line.jpg", plot = pub_year_geograph_line, device = 'jpg', width = 168, height = 100, units = "mm",
#       dpi = 300, limitsize = TRUE)
...
#ggsave("BioRegion_PublicationYear_heat.jpg", plot = BioRegion_PublicationYear_heat, device = 'jpg', width = 168, height = 100, units = "mm",
#       dpi = 300, limitsize = TRUE)

```


### Heatmap PFAS by Publicaion year
```{r pressure, echo=FALSE}
...
#ggsave("pfas_PublicationYear_heat.jpg", plot = pfas_PublicationYear_heat, device = 'jpg', width = 168, height = 100, units = "mm",
#      dpi = 300, limitsize = TRUE)
```
...
ggplot(Habitat_Species_count, aes(x= reorder(Habitat, -n), y = reorder(Species_higher_taxon, -n), size = n)) +
  ...
  scale_x_discrete(limits = c("Marine", "Freshwater", "Terrestrial inland",  "Terrestrial coastal", "Mixed", "Estuarine"))
...
Region_Species_bubble <- ggplot(Region_Species_count, aes(x= reorder(Biogeographical_region, -n), y = reorder(Species_higher_taxon, -n), size = n)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_x_discrete(limits = c( "Temperate",   "Subtropical", "Polar",  "Mixed","Tropical"))
...
Sampling_period_plot <- ggplot(Sampling_Period_Class_no_NA_SHORT_2, aes(x=Species_higher_taxon, y=SamplingPeriod,)) + 
  geom_violin() +
...
#ggsave("Sampling_period_plot.jpg", plot = Sampling_period_plot, device = 'jpg', width = 200, height = 100, units = "mm", dpi = 300, limitsize = TRUE)
